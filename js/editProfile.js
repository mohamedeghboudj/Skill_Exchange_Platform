...
document.addEventListener('DOMContentLoaded', function () {
    ...
    let resetBtn = document.querySelector("#reset")
    let isTeacherModeActive = false;
    let wasTeacherBefore = false; // ADDED: Track if user was originally a teacher
    let currentProfilePicture = ''; // ADDED
    ...
    // MODIFIED: Added confirmation when disabling teacher mode
    teachingCheckbox.addEventListener('change', function () {
    ...
    function populateForm(user) {
        ...
        // MODIFIED: Teacher mode - Track original status
        if (user.is_teacher && user.is_teacher == 1) {
            isTeacherModeActive = true;
            wasTeacherBefore = true; // ADDED: Track original status
            role.value = "Teacher";
            teachingCheckbox.checked = true;
        } else {
            isTeacherModeActive = false;
            wasTeacherBefore = false; // ADDED
            role.value = "Student";
            teachingCheckbox.checked = false;
        }
    ...
    // ADDED - save to backend function
